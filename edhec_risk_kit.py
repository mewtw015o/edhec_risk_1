...
def get_ffme_returns(col_list=['Lo 10','Hi 10']):
...
def is_normal(r,level=0.01):
	...
	return p_value > level #Hypothesis Null: it is normal if p_value > level
...
def minimize_vol(target_return, er, cov):
    ...
    return_is_target = {
        ...
        'args' : (er,),
        'fun' : lambda weights, er : target_return - portfolio_return(weights, er) #lambda, given weights and er, calculate the following function
    }
    weights_sum_to_1 = {
        'type' : 'eq', #equal type; see function below
...
def gmv(cov):
	...
	"""
	n = cov.shape[0]
	return msr(0,np.repeat(1,n),cov)  #Risk Free and expected return do not matter in this case. Goal is to find weights that minimizing the vol 
...
def run_cppi(risky_r, safe_r=None, m=3, start=1000, floor=0.8, riskfree_rate=0.03, drawdown=None):	
	...
	#Set up some Dataframe for saving intermediate values
	...
	for step in range(n_steps):
		...
		risky_w = m*cushion
		risky_w = np.minimum(risky_w,1) #constraint
		risky_w = np.maximum(risky_w,0) #constraint
		safe_w = 1 - risky_w
		risky_alloc = account_value*risky_w
		safe_alloc = account_value*safe_w
		## Update the account value for this time step
		account_value = risky_alloc*(1 + risky_r.iloc[step]) + safe_alloc*(1 + safe_r.iloc[step])
		# save the values to pdframes
		cushion_history.iloc[step] = cushion
		risky_w_history.iloc[step] = risky_w 
		account_history.iloc[step] = account_value
	risky_wealth = start*(1+risky_r).cumprod()
	
	return {"Wealth" : account_history,
			"Risky Wealth" : risky_wealth,
			"Risk Budget" : cushion_history,
			"Risky Allocaton" : risky_w_history,
			"m": m,
			"start" : start,
			"floor" : floor,
			"risky_r" : risky_r,
...
def bond_cash_flows(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12):
	...
	coupon_times = np.arange(1,n_coupons+1) #last number in arange is not included in the Series
	cash_flows = pd.Series(data=coupon_amt, index=coupon_times)
	cash_flows.iloc[-1] += principal
	return cash_flows
	
# def old_bond_price(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12, discount_rate=0.03):
	# """
	# Price a bond based on bond parameters maturity, prinicpal, coupon rate, and coupons per year
...
def macaulay_duration(flows, discount_rate):
	...
	"""
	discounted_flows = discount(flows.index, discount_rate)*flows  #PV of the cashflows
	weights = discounted_flows/discounted_flows.sum()
	return np.average(flows.index,weights=weights) #(flows.index*weights).sum()
...
def match_durations(cf_t, cf_s, cf_l, discount_rate):
	...
	"""
	d_t = macaulay_duration(cf_t, discount_rate) #Target
...
def bond_price(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12, discount_rate=0.03):
	...
	if isinstance(discount_rate, pd.DataFrame):
		...
		for t in pricing_dates:
			# t/coupons_per_year; Examples in the loop ... 1/12....2/12.....120/120
...
def bond_total_return(monthly_prices, principal, coupon_rate, coupons_per_year):
	...
	and the dividends are reinvested in the bond
	"""
...
def terminal_stats(rets, floor = 0.8, cap=np.inf, name="Stats"):
	...
	rets is a T x N DataFrame of returns, where T is the time-step (we asumme rets is sorted by time)
	Returns a 1 column DataFrame of Summary Stats indexed by the stat name
	"""
	terminal_wealth = (rets+1).prod()
	breach = terminal_wealth < floor #Return a series of 
...
def drawdown_allocator(psp_r, ghp_r, maxdd, m=3):
	"""
	Allocate beteen PSP and GHP with the goal to provide exposure to the upside
